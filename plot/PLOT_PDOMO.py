...
def regret_plot(eps_list, recur_list, number_of_clients, src_dir_name, dst_name, topology, alpha_dict=None):
    ...
    if(alpha_dict==None):
        ...
        alpha = alpha_dict['PDOMO']
    for epsilon in eps_list:
        for recursion in recur_list:
            file_name = src_dir_name + '/PDOMO_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_' +\
    ...
    ############################################################
    # PDOCO
    PDOCO_class_err_mean = []
    if (alpha_dict == None):
        alpha = 0.0
    else:
        alpha = alpha_dict['PDOCO']
    for epsilon in eps_list:
        for recursion in recur_list:
            file_name = src_dir_name + '/PDOCO_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_' +\
                            repr(epsilon) + '_' + topology
            class_err_list = []
            with open(file_name) as fd:
                for line in fd.readlines():
                    tmp = line.split(" ")
                    class_err_list.append(float(tmp[1][:-1]))
            PDOCO_class_err_mean.append(np.mean(class_err_list))
    PDOCO_class_err_mean = np.reshape(PDOCO_class_err_mean, (len(eps_list), len(recur_list)))
    ############################################################
    # DOCO
    DOCO_class_err_mean = []
    if (alpha_dict == None):
        ...
        alpha = alpha_dict['DOCO']
    for recursion in recur_list:
        file_name = src_dir_name + '/DOCO_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_' + topology
        class_err_list = []
        with open(file_name) as fd:
            for line in fd.readlines():
                tmp = line.split(" ")
                class_err_list.append(float(tmp[1][:-1]))
        DOCO_class_err_mean.append(np.mean(class_err_list))
    DOCO_class_err_mean = np.array(DOCO_class_err_mean)
    ############################################################
    ...
    plt.plot(recur_list, DOCO_class_err_mean, linewidth=param_linewidth, marker="s",
             markersize=param_markersize_, c=color[0])

    plt.plot(recur_list, PDOCO_class_err_mean[0, :], '--', linewidth=param_linewidth,
             marker="o", markersize=param_markersize_, c=color[1])
    plt.plot(recur_list, PDOCO_class_err_mean[1, :], '-.', linewidth=param_linewidth,
             marker="v", markersize=param_markersize_, c=color[1])
    plt.plot(recur_list, PDOCO_class_err_mean[2, :], ':', linewidth=param_linewidth,
...
def regret_plot_64(eps_list, recur_list, number_of_clients, src_dir_name, dst_name, topology_list, alpha_dict=None):
            ...
            for recursion in recur_list:
                ...
                with open(file_name) as fd:
                    for line in fd.readlines():
                        ...
                        class_err_list.append(float(tmp[1][:-1]))
                PDOCO_class_err_mean.append(np.mean(class_err_list))
    PDOCO_class_err_mean = np.reshape(PDOCO_class_err_mean, (len(topology_list), len(recur_list)))
    ############################################################
    # DOCO
    DOCO_class_err_mean = []
    if (alpha_dict == None):
        ...
        for recursion in recur_list:
            ...
            with open(file_name) as fd:
                for line in fd.readlines():
                    ...
                    class_err_list.append(float(tmp[1][:-1]))
            DOCO_class_err_mean.append(np.mean(class_err_list))
    DOCO_class_err_mean = np.reshape(DOCO_class_err_mean, (len(topology_list), len(recur_list)))
    ...
    plt.plot(recur_list, DOCO_class_err_mean[0, :], '--', linewidth=param_linewidth,
             marker="o", markersize=param_markersize_, c=color[0])
    plt.plot(recur_list, DOCO_class_err_mean[1, :], '-.', linewidth=param_linewidth,
...
if (__name__ == '__main__'):
    ...
    for data_source in data_source_list:
        conf_dict = conf_load('conf-dir/conf-obli-' + data_source + '-8.ini')
    ...
    src_dir_name = '../adv_setting/plot_data'
    dst_name = 'PDOMO-' + data_source + '-64-cvx.pdf'
    regret_plot_64(eps_list, recur_list, number_of_clients, src_dir_name, dst_name, topology_list)

    src_dir_name = '../adv_setting/plot_data'
    dst_name = 'PDOMO-' + data_source + '-64-strong.pdf'
