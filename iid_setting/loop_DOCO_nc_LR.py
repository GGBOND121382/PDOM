...
def run():
    ...
    for recursion in recur_list:
        file_name = './plot_data/non_pri_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_' +\
                    topology
        fd = open(file_name, 'a')
        ...
        for rpt in range(1):
            ...
            clf = PDOMO(eps=None, is_private=False, num_client=number_of_clients, alpha=alpha, is_comm=False)

            clf.fit(X_train[:recursion, :], y_train[:recursion, :], None)
