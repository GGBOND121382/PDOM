...
def run():
    ...
    alpha_g = alpha_dict['PDOMS']
    # for alpha in [0.0, alpha_g]:
    for alpha in [alpha_g, ]:
        for epsilon in eps_list:
            for recursion in recur_list:
                print("start.....")
                file_name = './plot_data/PDOMS_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_'\
                ...
                for rpt in range(rpt_times):
                    ...
                    clf = PDOMS(eps=epsilon, num_client=number_of_clients, alpha=alpha)
