...
from optimization_utils.LogisticRegression import projection
...
class PTOFW():
    ...
    def fit(self, X, y):
        ...
        pre_w = copy.deepcopy(self.w)
        # g_1 = []
        # g_2 = []
        G_t = []
        for i in range(self.num_client):
            # g_1.append(np.zeros((n_features + 1, 1)))
            # g_2.append(np.zeros((n_features + 1, 1)))
            G_t.append(np.zeros((n_features + 1, 1)))
        ...
        # C_norm = math.sqrt(d)
        C_norm = self.radius
        sigma = 2 * (C_norm * L_G + L_cons) * math.sqrt(d) * math.ceil(math.log(num_dataofclient, 2)) / self.eps  # in PFTAL, sigma depends on log(T, 2), the sensitivity for the sum of gradients is 2L
        ...
        if self.gradient == True:
            ...
            while True:
                ...
                for i in range(num1, num2):
                    ...
                    if(tt > 0):
                        h_x = x_train.dot(pre_w[i - num1])
                        h_x = np.array(h_x)
                        # print("h_x shape:", h_x.shape)
                        y_pred = sigmoid(h_x[0, 0])
                        pre_w_grad = x_train.T * (y_pred - y_train)
                    else:
                        pre_w_grad = np.zeros((n_features + 1, 1))

                    # print("i:", i - num1)
                    # print(w_grad.shape)
                    # print(pre_w_grad.shape)
                    G_t[i - num1] += (tt + 2) * w_grad - (tt + 1) * pre_w_grad

                    random_noise = noise_tree[i - num1]()
                    # random_noise = np.zeros((d, 1))

                    noisy_G = G_t[i - num1] + random_noise
                    d_t = noisy_G / (tt + 2)
                    v_t = -d_t / np.linalg.norm(d_t) * self.radius
                    pre_w[i - num1] = self.w[i - num1]
                    self.w[i - num1] = self.w[i - num1] + 1 / (tt + 2) * (v_t - self.w[i - num1])
