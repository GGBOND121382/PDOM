...
def run():
    ...
    alpha_g = alpha_dict['PDOMO']
    # for alpha in alpha_list:
    for alpha in [0.0, alpha_g]:
        for epsilon in eps_list:
            for recursion in recur_list:
                print("start.....")
                file_name = './plot_data/PDOMO_' + source + '_' + repr(alpha) + '_' + repr(recursion) + '_' +\
                ...
                for rpt in range(rpt_times):
                    ...
                    clf = PDOMO(eps=epsilon, num_client=number_of_clients, alpha=alpha)
                    # A = two2three_cycle()
